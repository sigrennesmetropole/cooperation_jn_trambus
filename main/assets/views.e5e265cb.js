import { aL as defineStore, aM as ref, b7 as CesiumMap, b8 as Projection, b9 as Cartographic, ba as OpenlayersMap, bb as SceneTransforms, bc as Cartesian2, bd as getHeightFromTerrainProvider, be as mercatorProjection, bf as LineString, bg as Feature, bh as Point, bi as getMidPoint, bj as cartesian2DDistance, bk as CatmullRomSpline, bl as Cartesian3, bm as CesiumMath } from "./vendor.b604c758.js";
import { u as useLayersStore, R as RENNES_LAYER } from "./layers.b0101898.js";
...
const useLineInteractionStore = defineStore(
  ...
  () => {
    const selectedTrambusLines = ref([]);
    const selectedMetroLines = ref([]);
    const selectedBusLines = ref([]);
    const isBikeSelected = ref(false);
    const clickPosition = ref(null);
    const featureLabel = ref(null);
    function selectTrambusLines(lines) {
      selectedTrambusLines.value = lines;
    }
    function selectMetroLines(lines) {
      selectedMetroLines.value = lines;
    }
    function selectBusLines(lines) {
