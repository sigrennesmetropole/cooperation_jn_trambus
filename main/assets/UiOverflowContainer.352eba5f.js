import { aw as defineComponent, aM as ref, aP as computed, aA as openBlock, aF as createElementBlock, aI as createBaseVNode, aR as renderSlot, aQ as normalizeClass } from "./vendor.2fe705a9.js";
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("div", _hoisted_1, [
    ...
    createBaseVNode("button", {
      class: normalizeClass(["absolute z-10 bg-white w-9 h-9 shadow-lg top-1/2 left-2 transform -translate-y-1/2 rounded-lg flex items-start p-2.5 hover:bg-neutral-100 hover:drop-shadow", { hidden: $setup.isMostLeftPosition }]),
      onClick: _cache[0] || (_cache[0] = ($event) => $setup.scroll(-250))
    }, [
      createBaseVNode("img", { src: $setup.leftArrow }, null, 8, _hoisted_2)
    ], 2),
    createBaseVNode("button", {
      class: normalizeClass(["absolute z-10 bg-white w-9 h-9 shadow-lg top-1/2 right-2 transform -translate-y-1/2 rounded-lg flex items-start p-2.5 hover:bg-neutral-100 hover:drop-shadow", { hidden: $setup.isMostRightPosition }]),
